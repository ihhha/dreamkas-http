...
class TerminalServiceSpec extends FlatSpec with Matchers with EitherValues {
  ...
  "Out" should "error incorrect input" in {
    TerminalService.processOut(incorrectData, packetIndex).left.value shouldBe UnknownFormat
  }

  it should "error no ETX" in {
    TerminalService.processOut(noEtxData, packetIndex).left.value shouldBe NoEtxFound
  }

  it should "process response" in {
    TerminalService.processOut(correctData1, packetIndex).right.value shouldBe ()
    TerminalService.processOut(correctData2, packetIndex + 1).right.value shouldBe ()
  }

  it should "return error" in {
    TerminalService.processOut(correctDataWithError, packetIndex).left.value shouldBe FunctionUnavailableWithSuchStatus
  }
